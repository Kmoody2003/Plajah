  ...
  useEffect(() => {
    const unsubscribe = onAuthUpdate(async (u) => {
      ...
      if (u) {
        ...
        // Reserve this account a persistent guide CHANNEL NUMBER at login (idempotent). Having a
        // number does NOT list them — their channel only appears when they have live feeds or an
        // active FAST channel; otherwise it stays unlisted.
        allocateChannelNumber(u.uid).catch(() => {});
                  ...
                  <header className="mb-12 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mx-auto lg:mx-0 max-w-2xl mb-6 rounded-3xl bg-white/[0.04] border border-white/10 backdrop-blur-md px-6 py-5 sm:px-8 sm:py-6 text-center space-y-3 shadow-[0_10px_40px_-20px_rgba(0,0,0,0.6)]">
                        <p className="text-white/75 text-[13px] sm:text-[15px] leading-relaxed text-balance">Plajah is <span className="text-small-orange font-semibold">one connected world for everyone who creates</span> — stream music, film and books and meet the people who made them, then keep building: learn in <span className="text-white font-semibold">Academia</span>, gather in <span className="text-white font-semibold">Elevate</span>, sell in <span className="text-white font-semibold">Business</span>, and preserve culture in our <span className="text-white font-semibold">Museums</span>.</p>
