...
/** Append one slot to the account's FAST channel schedule (creating + publishing it on first use). */
const appendFastSlot = async (uid: string, slot: FastChannelSlot): Promise<void> => {
  const existing = await fetchFastChannelSchedule(uid).catch(() => null);
  const schedule: FastChannelSchedule = (existing && Array.isArray(existing.slots))
    ? { ...existing, slots: [...existing.slots, { ...slot, order: existing.slots.length }] }
    : { userId: uid, slots: [{ ...slot, order: 0 }], adFrequencyMinutes: 20, adDurationSeconds: 60, commercialFree: false, loopSchedule: true, autoGenerated: false, includePublicDomain: false } as FastChannelSchedule;
  await saveFastChannelSchedule(schedule);
  await updateFastChannelEnabled(uid, true).catch(() => {});
  await saveFastChannelMeta({ ownerId: uid, isPublished: true }).catch(() => {});
  await allocateChannelNumber(uid).catch(() => {});
...
/** Add a Reello video to the account's FAST channel (from the Asset Manager). Live recordings get the
 *  REPLAY bug; everything else airs as a normal programme. */
export const addVideoToFastChannel = async (uid: string, video: Partial<Video>): Promise<void> => {
  ...
  try {
    ...
    await appendFastSlot(uid, {
      ...
      videoId: video.id,
      videoUrl: (video as any).muxPlaybackId ? `https://stream.mux.com/${(video as any).muxPlaybackId}.m3u8` : (video.url || ''),
      videoTitle: video.title || 'Programme',
      videoThumbnail: (video as any).thumbnailUrl || (video as any).coverImageUrl,
      videoDurationSeconds: durationSeconds,
      isReplay: isReplay || undefined,
      bugLabel: isReplay ? 'REPLAY' : undefined,
    });
  } catch (e) { console.warn('[fast] addVideoToFastChannel failed', e); }
};

/** Add a live source (a Reello live stream) to the account's FAST channel as a scheduled LIVE slot. */
export const addLiveToFastChannel = async (uid: string, live: { streamId?: string; url?: string; title?: string; maxDurationSeconds?: number }): Promise<void> => {
  if (!uid) return;
  try {
    await appendFastSlot(uid, {
      id: `live_${live.streamId || Date.now()}`, type: 'LIVE_INTERRUPT', order: 0,
      liveSourceKind: 'reello_live', liveSourceId: live.streamId, liveSourceUrl: live.url,
      liveSourceTitle: live.title || 'Live', videoTitle: live.title || 'Live',
      liveInterruptMaxDurationSeconds: live.maxDurationSeconds || 3600,
      bugLabel: 'LIVE',
    });
  } catch (e) { console.warn('[fast] addLiveToFastChannel failed', e); }
};
...
export const autoGenerateFastChannelSchedule = async (uid: string): Promise<FastChannelSchedule> => {
  ...
  // Build one programme block (intro/video/outro/ad) and account its real running time.
  let totalSec = 0;
  const DAY_SEC = 86400;
  ...
  const pushVideo = (video: any) => {
    ...
    if (video.adMarkers?.length) videoSlot.adMarkersSeconds = video.adMarkers.map((m: any) => m.time);
    slots.push(videoSlot); order++; totalSec += durationSec;
    if (outroBumper) {
      const bd = outroBumper.durationSeconds || 5;
      slots.push({ id: `slot_${order}`, type: 'BUMPER', order, bumperId: outroBumper.id, bumperUrl: outroBumper.url, bumperTitle: outroBumper.title, bumperDurationSeconds: bd });
      order++; totalSec += bd;
    }
