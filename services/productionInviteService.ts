...
export async function acceptProductionInvite(token: string): Promise<{ productionId: string; productionTitle: string }> {
  ...
  return runTransaction(db, async tx => {
    ...
    const member: ProductionMember & { inviteToken: string } = { id: u.uid, uid: u.uid, name: u.displayName || 'Plajah member', role: invite.position, roleKey: invite.roleKey as ProductionRoleKey, dept: invite.department, isCast: invite.roleKey === 'CAST', status: 'ACTIVE', createdAt: Date.now(), inviteToken: token };
    ...
    return { productionId: invite.productionId, productionTitle: invite.productionTitle };
  });
}
