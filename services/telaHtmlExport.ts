import type { TelaBaseDevice, TelaDoc, TelaFrame, TelaImageDevice, TelaMediaDevice, TelaVectorDevice, TelaVectorObject } from '../types';
...
function renderImage(device: TelaImageDevice) {
  return `<div class="tela-page-layer">${device.layers.filter(layer => layer.visible).map(layer => {
    const style=`position:absolute;left:${layer.x}px;top:${layer.y}px;transform-origin:0 0;transform:rotate(${layer.rotation || 0}deg) scale(${layer.scale});opacity:${layer.opacity};mix-blend-mode:${safeCss(layer.blend)};filter:brightness(${layer.adjust.brightness}) contrast(${layer.adjust.contrast}) saturate(${layer.adjust.saturate}) blur(${layer.adjust.blur}px)`;
    if (layer.mediaKind === 'LOTTIE') return `<canvas class="tela-lottie" data-src="${esc(layer.src)}" data-width="${layer.intrinsicWidth || 512}" data-height="${layer.intrinsicHeight || 512}" aria-label="${esc(layer.name || 'Animated graphic')}" style="${style};width:${layer.intrinsicWidth || 512}px;height:${layer.intrinsicHeight || 512}px"></canvas>`;
    if (layer.mediaKind === 'VIDEO') return `<video src="${esc(layer.src)}" autoplay muted loop playsinline aria-label="${esc(layer.name || '')}" style="${style}"></video>`;
    return `<img alt="${esc(layer.name || '')}" src="${esc(layer.src)}" style="${style}"/>`;
  }).join('')}</div>`;
}
...
function renderMedia(device: TelaMediaDevice) {
  const src=esc(device.src), name=esc(device.name);
  if(device.kind==='IMAGE')return `<img class="tela-media" src="${src}" alt="${name}">`;
  if(device.kind==='AUDIO')return `<div class="tela-asset-card"><strong>${name}</strong><audio src="${src}" controls></audio></div>`;
  if(device.kind==='VIDEO')return `<video class="tela-media" src="${src}" controls playsinline></video>`;
  if(device.kind==='PDF')return `<object class="tela-media" data="${src}" type="application/pdf"><a href="${src}">Open ${name}</a></object>`;
  if(device.kind==='MODEL_3D'&&/\.(glb|gltf)$/i.test(device.name))return `<model-viewer class="tela-media" src="${src}" alt="${name}" camera-controls auto-rotate></model-viewer>`;
  return `<div class="tela-asset-card"><strong>${name}</strong><a href="${src}" download="${name}">Download asset</a></div>`;
}
