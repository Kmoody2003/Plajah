...
export function planPlayback(clips: any[], mediaPool: any[], t0: number): PlanEntry[] {
  ...
  for (const c of clips || []) {
    ...
    out.push({
      clipId: c.id, url: asset.url, trackId: c.trackId,
...
async function decodeUrl(url: string): Promise<AudioBuffer | null> {
  ...
  const p = (async () => {
    try {
      ...
      console.warn('[playbackEngine] cannot decode — element fallback for', url, (e as any)?.message || e);
      unplayable.add(url);
...
export function engineClock(): number {
  ...
  return state.t0 + Math.max(0, ctx.currentTime - state.ctxStart);
}
