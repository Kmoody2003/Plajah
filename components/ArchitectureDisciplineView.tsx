    ...
    <button onClick={onOpen} className="group text-left rounded-[1.4rem] overflow-hidden border border-white/8 bg-white/[0.03] hover:bg-white/[0.06] transition-all"
      ...
      <div className="aspect-[4/3] bg-gradient-to-b from-white/5 to-black/40 relative overflow-hidden">
        ...
        <div className="absolute bottom-2.5 left-3 right-3">
          ...
          <p className={`${TYPE.labelSm} font-bold uppercase tracking-widest mt-1`} style={{ color: ACCENT }}>{style.era}{style.region ? ` · ${style.region}` : ''}</p>
        </div>
      </div>
    </button>
  );
      ...
      <motion.div initial={{ scale: 0.94, y: 12 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.96, opacity: 0 }} onClick={e => e.stopPropagation()}
        ...
        <div className="p-5 -mt-10 relative">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            <span className={`px-2.5 py-1 rounded-full ${TYPE.labelSm} font-black uppercase`} style={{ background: `${ACCENT}26`, border: `1px solid ${ACCENT}4d`, color: ACCENT }}>{style.era}</span>
            {style.region && <span className={`px-2.5 py-1 rounded-full bg-white/5 border border-white/10 ${TYPE.labelSm} font-black uppercase text-white/50`}>{style.region}</span>}
          </div>
          ...
          <div className="mt-4">
            <p className={`${TYPE.labelSm} font-black uppercase tracking-[0.3em] text-white/35 mb-2`}>Hallmarks</p>
            <div className="flex flex-wrap gap-1.5">
              {style.hallmarks.map(h => <span key={h} className="px-2.5 py-1 rounded-lg bg-white/[0.04] border border-white/8 type-body-sm text-white/70">{h}</span>)}
            </div>
          </div>
...
const FormulaCard: React.FC<{ f: ArchFormula }> = ({ f }) => (
  <div className="rounded-2xl border border-white/8 bg-white/[0.03] p-4">
    <div className="flex items-center justify-between gap-2">
      <p className="type-body-md font-black text-white">{f.name}</p>
    ...
    <p className="type-body-sm text-white/50 leading-relaxed">{f.description}</p>
    ...
    <div className="min-h-screen text-white">
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="relative px-5 sm:px-6 pt-8 pb-4 max-w-7xl mx-auto">
          ...
          <p className={`${TYPE.labelSm} font-black uppercase tracking-[0.4em]`} style={{ color: ACCENT }}>Plajah Academia · Architecture</p>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-6 py-6">
            ...
            {ARCH_HISTORY.map((era, i) => (
              <motion.div key={era.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                ...
                <div className="flex items-baseline justify-between gap-3 flex-wrap">
                  ...
                  <span className={`${TYPE.labelSm} font-black uppercase tracking-widest`} style={{ color: ACCENT }}>{era.span}</span>
                </div>
                <p className="type-body-md text-white/55 leading-relaxed mt-2">{era.essay}</p>
                <div className="grid sm:grid-cols-2 gap-4 mt-3">
                  <div>
                    <p className={`${TYPE.labelSm} font-black uppercase tracking-[0.3em] text-white/35 mb-1.5`}>Key Buildings</p>
                    <ul className="space-y-1">{era.keyBuildings.map(b => <li key={b} className="type-body-sm text-white/55 flex gap-2"><span style={{ color: ACCENT }}>·</span>{b}</li>)}</ul>
                  </div>
                  <div>
                    <p className={`${TYPE.labelSm} font-black uppercase tracking-[0.3em] text-white/35 mb-1.5`}>Developments</p>
                    <ul className="space-y-1">{era.developments.map(b => <li key={b} className="type-body-sm text-white/55 flex gap-2"><span style={{ color: ACCENT }}>·</span>{b}</li>)}</ul>
                  </div>
                </div>
              </motion.div>
            ))}
        ...
        {tab === 'formulas' && (
          <div className="space-y-6">
            ...
            <div>
              <p className={`${TYPE.labelSm} font-black uppercase tracking-[0.3em] text-white/40 mb-3`}>Building Codes &amp; Standards</p>
                ...
                {ARCH_CODES.map(c => (
                  <a key={c.name} href={c.url} target="_blank" rel="noreferrer" className="rounded-2xl border border-white/8 bg-white/[0.03] p-4 hover:bg-white/[0.06] transition-all block">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className={`${TYPE.labelSm} font-black uppercase tracking-widest text-white/35 shrink-0`}>{c.org}</span>
                    </div>
                    <p className="type-body-sm text-white/45 mt-1">{c.scope}</p>
        ...
        {tab === 'toolbox' && (
          <div className="space-y-6">
            <div>
              <p className={`${TYPE.labelSm} font-black uppercase tracking-[0.3em] text-white/40 mb-3`}>Open-Source &amp; Industry Software</p>
        ...
        {tab === 'library' && (
          <div className="space-y-6">
            <div>
              <p className={`${TYPE.labelSm} font-black uppercase tracking-[0.3em] text-white/40 mb-3`}>Textbooks &amp; Foundational Texts</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
