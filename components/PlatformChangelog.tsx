...
import { BUILD_LOG } from '../data/buildLog.generated';
  ...
  return (
    <div className="w-full h-full overflow-y-auto custom-scrollbar bg-black text-white">
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        {/* Recent builds — the raw technical feed straight from git, so this page is never stale even
            between curated notes. Auto-generated by scripts/genBuildLog.mjs. */}
        {BUILD_LOG.length > 0 && (
          <div className="mt-6">
            <button
            ...
            >
              <span className="text-[10px] font-black uppercase tracking-[0.25em] text-white/40">Recent builds</span>
              <span className="flex-1 h-px bg-white/8" />
              <span className="text-[9px] text-white/25 font-bold">{BUILD_LOG.length} commits</span>
              <ChevronDown size={14} className={`text-white/30 transition-transform ${showBuilds ? 'rotate-180' : ''}`} />
            </button>
            {showBuilds && (
              <div className="mt-2 rounded-2xl border border-white/8 bg-white/[0.02] divide-y divide-white/[0.05]">
                {BUILD_LOG.map(b => (
                  <div key={b.hash} className="flex items-center gap-3 px-4 py-2.5">
                    <span className="text-[9px] font-black uppercase tracking-widest text-[#8B5CF6]/80 w-16 shrink-0">{b.area}</span>
                    <span className="flex-1 text-[11px] text-white/60 leading-snug min-w-0">{b.subject}</span>
                    <span className="text-[9px] text-white/25 font-mono shrink-0">{b.date}</span>
                    <span className="text-[9px] text-white/20 font-mono shrink-0 hidden sm:inline">{b.hash}</span>
                  </div>
                ))}
              </div>
            )}
