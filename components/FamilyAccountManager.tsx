...
import { Baby, Plus, ShieldCheck, LogIn, Trash2, X, Loader2 } from 'lucide-react';
import type { UserProfile } from '../types';
import { createChildProfile, listChildProfiles, deleteChildProfile } from '../services/backendService';
...
const FamilyAccountManager: React.FC<{ guardianUid: string }> = ({ guardianUid }) => {
  ...
  const addChild = async () => {
    if (!name.trim()) return;
    setBusy(true);
    const child = await createChildProfile(guardianUid, { displayName: name.trim(), birthYear: birthYear ? parseInt(birthYear, 10) : undefined });
    setBusy(false);
    if (child) { setName(''); setBirthYear(''); setAdding(false); load(); }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        ...
        <button onClick={() => setAdding(a => !a)} className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-small-orange text-white text-[10px] font-black uppercase tracking-widest hover:brightness-110 transition-all">
          <Plus size={14} /> Add child
        </button>
      </div>

      {adding && (
        <div className="p-4 bg-white/[0.03] border border-white/10 rounded-2xl flex flex-wrap items-end gap-3">
          ...
          <button onClick={addChild} disabled={busy || !name.trim()} className="px-5 py-2.5 rounded-xl bg-white text-black text-[10px] font-black uppercase tracking-widest disabled:opacity-30">
          ...
          {children.map(c => (
            <div key={c.uid} className="p-4 bg-white/[0.03] border border-white/10 rounded-2xl flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="font-black truncate">{c.displayName}</div>
