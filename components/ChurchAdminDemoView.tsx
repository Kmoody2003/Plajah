...
const KPI: React.FC<{ label: string; value: string; sub?: string; icon: React.ComponentType<{ size?: number; className?: string; style?: React.CSSProperties }> }> = ({ label, value, sub, icon: Icon }) => (
  <div className="rounded-2xl p-4 bg-white/[0.03] border border-white/10">
    <div className="flex items-center justify-between mb-2"><span className="text-[9px] font-black uppercase tracking-widest text-white/35">{label}</span><Icon size={14} style={{ color: VS }} /></div>
    <p className="text-2xl font-black text-white tabular-nums">{value}</p>
    {sub && <p className="text-[10px] font-bold mt-0.5" style={{ color: VS }}>{sub}</p>}
  </div>
);
const Btn: React.FC<{ children: React.ReactNode; primary?: boolean; onClick?: () => void }> = ({ children, primary, onClick }) => (
  <button onClick={onClick || nudge}
    className={`flex items-center gap-1.5 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${primary ? 'text-white hover:brightness-110' : 'bg-white/5 border border-white/10 text-white/60 hover:text-white'}`}
    style={primary ? { background: V } : undefined}>{children}</button>
);
const H: React.FC<{ children: React.ReactNode; sub?: string }> = ({ children, sub }) => (
  <div className="mb-5"><h2 className="text-xl font-black uppercase tracking-tight text-white">{children}</h2>{sub && <p className="text-[11px] text-white/40 mt-0.5">{sub}</p>}</div>
);
  ...
  return (
    <div className="h-full overflow-hidden flex flex-col bg-[#0d0b12]">
      ...
      <div className="shrink-0 flex items-center gap-3 px-5 py-2.5 border-b border-white/8" style={{ background: 'rgba(139,92,246,0.10)' }}>
        <span className="text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-md text-white" style={{ background: V }}>Demo Backend</span>
        <p className="text-[11px] font-bold text-white/60 truncate">{c.name} · Ministry admin — this is what running your church looks like</p>
        <button onClick={onCreate} className="ml-auto shrink-0 flex items-center gap-1.5 px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest text-white" style={{ background: V }}>Create your church <ArrowRight size={12} /></button>
      </div>
      ...
      <div className="flex-1 min-h-0 flex">
        ...
        {/* Content */}
        <div className="flex-1 min-w-0 overflow-y-auto scrollbar-hide p-5 lg:p-8">
          {sec === 'OVERVIEW' && (
            <div className="max-w-4xl">
              ...
              <div className="grid lg:grid-cols-2 gap-4">
                <div className="rounded-2xl p-4 bg-white/[0.03] border border-white/10">
                  <p className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-3">Recent giving</p>
                  {GIFTS.slice(0, 4).map(g => (
                    <div key={g.id} className="flex items-center justify-between py-2 border-b border-white/6 last:border-0">
                      <div><p className="text-xs font-bold text-white">{g.name}</p><p className="text-[9px] text-white/35 uppercase tracking-widest">{g.fund}{g.recurring ? ' · recurring' : ''}</p></div>
                      <span className="text-sm font-black tabular-nums" style={{ color: VS }}>{money(g.amount)}</span>
                    </div>
                  ))}
                </div>
                <div className="rounded-2xl p-4 bg-white/[0.03] border border-white/10">
                  <p className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-3">Quick actions</p>
            ...
            <div className="max-w-2xl">
              ...
              <div className="space-y-3 mb-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {['In-app', 'Push', 'Email'].map(ch => <span key={ch} className="flex items-center gap-1.5 px-3 py-2 rounded-2xl text-[10px] font-black uppercase tracking-widest text-white" style={{ background: 'rgba(139,92,246,0.15)', border: '1px solid rgba(139,92,246,0.3)' }}><Check size={12} /> {ch}</span>)}
                </div>
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-3">Recently sent</p>
              <div className="space-y-2">
                {MESSAGES_SENT.map(m => (
                  <div key={m.id} className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-white/[0.03] border border-white/10">
                    ...
                    <div className="min-w-0 flex-1"><p className="text-xs font-bold text-white truncate">{m.subject}</p><p className="text-[9px] text-white/35 uppercase tracking-widest">{m.when} · {m.channels}</p></div>
                    <span className="text-[10px] font-black tabular-nums text-white/50 shrink-0">{m.reach} reached</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {sec === 'GIVING' && (
            <div className="max-w-3xl">
              ...
              {funds.length > 0 && (
                <>
                  <p className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-3">Funds</p>
                      ...
                      return (
                        <div key={f.id} className="rounded-2xl p-4 bg-white/[0.03] border border-white/10">
                          ...
                          {f.goal ? (<>
                            ...
                            <div className="flex justify-between mt-1.5 text-[10px] font-bold uppercase tracking-widest text-white/40 tabular-nums"><span style={{ color: VS }}>{money(f.raised || 0)}</span><span>of {money(f.goal)}</span></div>
              ...
              <p className="text-[10px] font-black uppercase tracking-widest text-white/40 mb-3">Recent gifts</p>
              <div className="rounded-2xl border border-white/10 overflow-hidden">
                {GIFTS.map(g => (
                  <div key={g.id} className="flex items-center gap-3 px-4 py-3 border-b border-white/6 last:border-0 hover:bg-white/[0.02]">
                    <div className="min-w-0 flex-1"><p className="text-xs font-bold text-white truncate">{g.name}</p><p className="text-[9px] text-white/35 uppercase tracking-widest">{g.fund} · {g.when}{g.recurring ? ' · recurring' : ''}</p></div>
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {DEMO_CHURCH_EVENTS.map((e, i) => (
                  <div key={e.id} className="rounded-2xl overflow-hidden bg-white/[0.03] border border-white/10">
                    ...
                    <div className="p-4">
                      <p className="text-[9px] font-black uppercase tracking-widest mb-1" style={{ color: VS }}>{e.date} · {e.time}</p>
                ...
                {DEMO_CHURCH_PRAYERS.map(pr => (
                  <div key={pr.id} className="rounded-2xl p-4 bg-white/[0.03] border border-white/10">
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest" style={{ color: VS }}><HandHeart size={11} /> {pr.praying}</span>
                    </div>
              ...
              <div className="space-y-2">
                {DEMO_CHURCH_STAFF.map(s => (
                  <div key={s.id} className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-white/[0.03] border border-white/10">
                    ...
                    <div className="min-w-0 flex-1"><p className="text-sm font-black text-white truncate">{s.name}</p><p className="text-[10px] font-bold uppercase tracking-widest" style={{ color: VS }}>{s.role}</p></div>
                    <select onChange={nudge} className="shrink-0 bg-black/40 border border-white/10 rounded-xl px-3 py-2 text-[10px] font-black uppercase tracking-widest text-white/70 outline-none">
          ...
          {sec === 'MULTISITE' && (
            <div className="max-w-3xl">
              ...
              <div className="aspect-video rounded-3xl overflow-hidden bg-black border border-white/10 relative mb-5 grid place-items-center">
                <img src={DEMO_CHURCH_EVENTS[1]?.image} className="absolute inset-0 w-full h-full object-cover opacity-40" alt="" />
                <span className="absolute top-3 left-3 flex items-center gap-1.5 px-2.5 py-1 bg-red-600 rounded-full text-[9px] font-black uppercase tracking-widest text-white"><Dot className="animate-pulse" size={14} /> Live · Downtown</span>
                <button onClick={nudge} className="relative z-10 flex items-center gap-2 px-5 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest text-white" style={{ background: V }}><MonitorPlay size={14} /> Take to switcher</button>
              </div>
              <div className="grid sm:grid-cols-3 gap-3">
                {CAMPUSES.map((cam, i) => (
                  <button key={cam.id} onClick={nudge} className="p-4 rounded-2xl bg-white/[0.03] border border-white/10 hover:bg-white/[0.06] text-left transition-all">
                    ...
                    <p className="text-[9px] font-black uppercase tracking-widest mt-2 flex items-center gap-1" style={{ color: i === 0 ? '#f87171' : 'rgba(255,255,255,0.3)' }}>{i === 0 ? <><Dot className="animate-pulse" size={12} /> On air</> : 'Idle'}</p>
                  </button>
                ))}
              </div>
            </div>
          ...
          {sec === 'CLASSES' && (
            <div className="max-w-2xl">
              ...
              <div className="space-y-2">
                {CLASSES.map(cl => (
                  <div key={cl.id} className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-white/[0.03] border border-white/10">
                    ...
                    <div className="min-w-0 flex-1"><p className="text-sm font-black text-white truncate">{cl.title}</p><p className="text-[9px] text-white/35 uppercase tracking-widest">{cl.enrolled} enrolled · {cl.lessons} lessons</p></div>
                    <span className="text-[8px] font-black uppercase tracking-widest px-2 py-1 rounded-full shrink-0" style={{ color: cl.status === 'Active' ? '#4ade80' : VS, background: cl.status === 'Active' ? 'rgba(74,222,128,0.12)' : 'rgba(139,92,246,0.14)' }}>{cl.status}</span>
                  </div>
                ))}
              </div>
              <div className="mt-6 rounded-2xl p-4 flex items-center gap-3 bg-white/[0.03] border border-white/10">
                ...
                <p className="text-[11px] text-white/50">Classes run on Plajah's full EdTech stack — lessons, assignments, grades and a learning record — the same one schools use.</p>
              </div>
            </div>
          )}
        </div>
