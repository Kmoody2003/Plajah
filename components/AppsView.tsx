        ...
        <AnimatePresence mode="wait">
          {selectedApp ? (
            ...
            >
              ...
              {/* Native platform app — Tela (unified document canvas) */}
              {activeTab === 'DISCOVER' && ('tela document canvas word processor spreadsheet form database design writer grid'.includes(searchQuery.toLowerCase()) || searchQuery === '') && (
                <div
                  key="native-tela"
                  onClick={() => window.dispatchEvent(new CustomEvent('plajah:openTela', { detail: {} }))}
                  className="group cursor-pointer space-y-6"
                >
                  <div className="relative aspect-square rounded-[2.5rem] overflow-hidden border border-[#00DAF3]/25 shadow-2xl transition-all group-hover:scale-105 group-hover:-translate-y-2 bg-[#0A0A0D]">
                    <div className="absolute inset-0" style={{ background: 'linear-gradient(135deg, rgba(107,0,153,0.30), rgba(0,218,243,0.18) 55%, rgba(255,140,0,0.15))' }} />
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 text-center p-8">
                      <LayoutPanelTop size={56} className="text-[#00DAF3] drop-shadow-[0_0_24px_rgba(0,218,243,0.5)]" />
                      <p className="text-2xl font-black uppercase tracking-tighter italic text-white">Tela</p>
                    </div>
                    <div className="absolute top-4 left-4 px-2.5 py-1 rounded-md bg-white/90 text-black text-[8px] font-black uppercase tracking-widest">Native</div>
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all flex flex-col items-center justify-center backdrop-blur-sm">
                      <Play fill="white" size={48} className="mb-4 text-white scale-75 group-hover:scale-100 transition-transform" />
                      <p className="text-[10px] font-black uppercase tracking-widest">Open canvas</p>
                    </div>
                  </div>
                  <div className="space-y-2 px-2">
                    <h3 className="text-lg font-black uppercase tracking-tight text-white truncate">Tela</h3>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-white/30">Docs · sheets · forms · design · one canvas</p>
                  </div>
                </div>
              )}
              ...
              {activeTab === 'DISCOVER' && ('melos beats daw drum machine step sequencer maschine groove sampler dawproject'.includes(searchQuery.toLowerCase()) || searchQuery === '') && (
                ...
                >
                  ...
                  <div className="space-y-2 px-2">
                    <h3 className="text-lg font-black uppercase tracking-tight text-white truncate">Melos Studio</h3>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-white/30">Pads · step sequencer · .dawproject</p>
                  </div>
                </div>
              )}

              {/* Native platform app — DJ Console (Traktor-class decks + streamer switcher, Chora-native) */}
              {activeTab === 'DISCOVER' && ('dj console decks mixer traktor turntable crossfader beatmatch scratch mix stream broadcast'.includes(searchQuery.toLowerCase()) || searchQuery === '') && (
                <div
                  key="native-dj-console"
                  onClick={() => window.dispatchEvent(new CustomEvent('OPEN_DJ_CONSOLE', { detail: {} }))}
                  className="group cursor-pointer space-y-6"
                >
                  <div className="relative aspect-square rounded-[2.5rem] overflow-hidden border border-[#00DAF3]/20 shadow-2xl transition-all group-hover:scale-105 group-hover:-translate-y-2 bg-[#0A0A0D]">
                    <div className="absolute inset-0 bg-gradient-to-br from-[#00DAF3]/20 via-transparent to-[#D40055]/20" />
                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 text-center p-8">
                      <Disc size={56} className="text-[#00DAF3] drop-shadow-[0_0_24px_rgba(0,218,243,0.5)]" />
                      <p className="text-2xl font-black uppercase tracking-tighter italic text-white">DJ</p>
                    </div>
                    <div className="absolute top-4 left-4 px-2.5 py-1 rounded-md bg-white/90 text-black text-[8px] font-black uppercase tracking-widest">Native</div>
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all flex flex-col items-center justify-center backdrop-blur-sm">
                      <Play fill="white" size={48} className="mb-4 text-white scale-75 group-hover:scale-100 transition-transform" />
                      <p className="text-[10px] font-black uppercase tracking-widest">Rock a set</p>
                    </div>
                  </div>
                  <div className="space-y-2 px-2">
                    <h3 className="text-lg font-black uppercase tracking-tight text-white truncate">DJ Console</h3>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-white/30">Decks · streamer switcher · Pixels</p>
                  </div>
                </div>
              )}
