...
interface FastChannelManagerProps {
  user: UserProfile;
  onBack: () => void;
  /** Open straight to a tab (e.g. 'SCHEDULE' from the radio builder) and/or a station (tv/radio). */
  initialTab?: Tab;
  initialStation?: 'tv' | 'radio';
}
...
const FastChannelManager: React.FC<FastChannelManagerProps> = ({ user, onBack, initialTab, initialStation }) => {
  const [activeTab, setActiveTab] = useState<Tab>(initialTab || 'CHANNEL');
  ...
  // Mon ≠ Tue ≠ … (the user then hand-tunes any day). Stored in weeklySlots; the playout + guide pick
  // the current weekday's list automatically.
  const handleAutoGenerateWeekly = async () => {
    setIsGenerating(true);
    try {
      const base = await autoGenerateFastChannelSchedule(user.uid).catch(() => null);
      const baseSlots = (base?.slots && base.slots.length ? base.slots : schedule?.slots) || [];
      if (baseSlots.length) {
        const weekly: Record<number, FastChannelSlot[]> = {};
        for (let d = 0; d < 7; d++) {
  ...
  // ── RADIO station (runs SIDE-BY-SIDE with the TV channel — the switch only picks which to edit) ──
  const [station, setStation] = useState<'tv' | 'radio'>(initialStation || 'tv');
  const [radioSchedule, setRadioSchedule] = useState<FastChannelSchedule | null>(null);
  const [radioTracks, setRadioTracks] = useState<Video[]>([]);
  const [radioLive, setRadioLive] = useState<LiveAudioSource[]>([]);
  const [radioLoaded, setRadioLoaded] = useState(false);

  useEffect(() => {
    if (station !== 'radio' || radioLoaded) return;
    (async () => {
      const [rs, albums] = await Promise.all([
        fetchRadioSchedule(user.uid).catch(() => null),
        fetchUserAlbums(user.uid).catch(() => [] as any[]),
      ]);
      // Music tracks → a Video-like library the scheduler understands (audio slots).
      const tracks: Video[] = (albums || [])
        .filter((a: any) => a.type === 'MUSIC' || !a.type)
        .flatMap((a: any) => (a.tracks || []).map((t: any) => ({
          id: t.id, title: t.title, url: t.url || t.audioUrl || '', duration: t.duration,
          thumbnailUrl: t.coverArt || t.artworkUrl, coverImageUrl: t.coverArt || t.artworkUrl,
        })))
        .filter((t: any) => t.url) as any;
      setRadioTracks(tracks);
      // Live audio sources: the account's active live feeds + its own Reello live, for LIVE slots.
      const live: LiveAudioSource[] = [];
      const lsc: any = (user as any).liveStreamConfig;
      if (Array.isArray(lsc?.liveFeeds) && lsc.liveFeeds.length) {
        lsc.liveFeeds.filter((f: any) => f.isActive && f.url).forEach((f: any) => live.push({ kind: 'reello_live', url: f.url, title: f.name || 'My Live Stream' }));
      } else if (lsc?.isActive && lsc?.streamUrl) {
        live.push({ kind: 'reello_live', url: lsc.streamUrl, title: 'My Live Stream' });
      }
      live.push({ kind: 'reello_live', title: 'My Reello Live (when on air)' });
      live.push({ kind: 'live_talk', title: 'Live Talk room (when open)' });
      setRadioLive(live);
      if (rs) setRadioSchedule(rs);
      setRadioLoaded(true);
    })();
  }, [station, radioLoaded, user.uid]);

  const handleAutoGenerateRadio = async () => {
    setIsGenerating(true);
    const g = await autoGenerateRadioSchedule(user.uid);
    setRadioSchedule(g);
    setIsGenerating(false);
  };
  const initRadioSchedule = () => setRadioSchedule({
    userId: user.uid, slots: [], adFrequencyMinutes: 20, adDurationSeconds: 30,
    commercialFree: false, loopSchedule: true, autoGenerated: false, includePublicDomain: false,
    stationKind: 'radio', lastUpdated: Date.now(),
  });
  const handleSaveRadioSchedule = async () => {
    if (!radioSchedule) return;
    setIsSaving(true);
    await saveRadioSchedule({ ...radioSchedule, stationKind: 'radio', autoGenerated: false });
    setIsSaving(false);
  };
          ...
          <motion.div key={activeTab} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.18 }}>
            ...
            {activeTab === 'SCHEDULE' && (
              <div className="space-y-6">
                {/* Station switch — TV channel and Radio station run SIDE-BY-SIDE (both active); this
                    only chooses which schedule you're editing. */}
                <div className="flex items-center gap-2">
                  <div className="flex rounded-2xl overflow-hidden border border-white/10">
                    {(['tv', 'radio'] as const).map(s => (
                      <button key={s} onClick={() => setStation(s)}
                        className={`px-5 py-2.5 text-[10px] font-black uppercase tracking-widest transition-colors flex items-center gap-2 ${station === s ? 'bg-small-orange text-white' : 'bg-white/5 text-white/40 hover:text-white/70'}`}>
                        {s === 'tv' ? <Tv size={13} /> : <Music2 size={13} />} {s === 'tv' ? 'TV Channel' : 'Radio Station'}
                      </button>
                    ))}
                  </div>
                  <span className="text-[8px] font-black uppercase tracking-widest text-white/25">Both run at once — switch to edit each</span>
                </div>
                ...
                {!schedule ? (
                  ...
                  <>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 p-3 rounded-2xl bg-white/[0.02] border border-white/10">
                      ...
                      <div className="flex flex-wrap gap-2">
                        ...
                        <button onClick={handleRetimeSchedule} disabled={isRetiming || !schedule}
                          ...
                          {isRetiming ? <RefreshCw size={12} className="animate-spin" /> : <Clock size={12} />} Fix Programme Times
                        </button>
                        <button onClick={handleAutoGenerateWeekly} disabled={isGenerating || myVideos.length === 0}
                          className="flex items-center gap-2 px-4 py-2 bg-white/5 border border-white/10 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-white/10 disabled:opacity-40">
                          {isGenerating ? <RefreshCw size={12} className="animate-spin" /> : <CalendarClock size={12} />} Generate 7-Day Week
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="p-5 bg-red-500/5 border border-red-500/20 rounded-2xl max-w-md">
                ...
                {station === 'radio' && (<>
                  {!radioSchedule ? (
                    <div className="py-24 flex flex-col items-center gap-6 border-2 border-dashed border-white/5 rounded-3xl">
                      ...
                      <div className="text-center">
                        <p className="text-white/40 font-black uppercase tracking-widest text-sm">No radio schedule yet</p>
                        <p className="text-white/20 text-[10px] uppercase tracking-widest mt-2">Auto-build from your radio-eligible tracks, or start manually</p>
                      </div>
                      <div className="flex gap-3">
                        <button onClick={handleAutoGenerateRadio} disabled={isGenerating} className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-[#6B0099] to-[#D40055] rounded-xl text-[9px] font-black uppercase tracking-widest disabled:opacity-40">
                          {isGenerating ? <RefreshCw size={13} className="animate-spin" /> : <Shuffle size={13} />} Auto-Generate
                        </button>
                        <button onClick={initRadioSchedule} className="flex items-center gap-2 px-6 py-3 bg-white/5 border border-white/10 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-white/10">
                          <Plus size={13} /> Start Manually
                        </button>
                      </div>
                    </div>
                    ...
                    <PlayoutScheduler
                      ...
                      liveSources={radioLive}
                      onSave={handleSaveRadioSchedule}
                      saving={isSaving}
                    />
                  )}
                </>)}
