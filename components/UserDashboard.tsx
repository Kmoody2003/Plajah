...
import { 
  User, Settings, Database, Video as VideoIcon, Music, Image as ImageIcon, 
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-12">
              <header className="flex items-end justify-between">
                ...
                <div className="flex gap-4">
                  ...
                  <button 
                    onClick={() => setShowCreator({ active: true, type: 'MUSIC' })}
              ...
              <div className="grid grid-cols-1 gap-4">
                {userAlbums.map(album => (
                  <div key={album.id} className="group flex items-center gap-6 p-6 bg-white/5 border border-white/5 rounded-[2rem] hover:bg-white/10 transition-all">
                    <div className="w-20 h-20 rounded-2xl overflow-hidden shrink-0">
                      <img src={album.coverImage || null} alt={album.title} className="w-full h-full object-cover" />
                    </div>
                ...
                {userAlbums.length === 0 && (
                  <div className="py-20 text-center opacity-20 border-2 border-dashed border-white/5 rounded-[3rem]">
                    ...
                    <p className="text-[10px] font-black uppercase tracking-widest">No assets deployed to the global archive.</p>
                  </div>
                )}
              </div>
            </motion.div>
