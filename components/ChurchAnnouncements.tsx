...
const ChurchAnnouncements: React.FC<{ orgId: string; orgName: string; orgPhoto: string; isOwner?: boolean }> = ({ orgId, orgName, orgPhoto, isOwner }) => {
  ...
  return (
    <section className="mt-10">
      ...
      {posts.length === 0 ? (
        <p className="type-body-sm text-white/30 uppercase tracking-widest text-center py-6">No announcements yet</p>
      ) : (
            ...
            return (
              <div key={p.id} className="rounded-2xl p-4 bg-white/[0.03] border border-white/10">
                <div className="flex items-center gap-2 mb-2">
                  <img src={p.authorPhoto || orgPhoto} className="w-8 h-8 rounded-full border border-white/10" alt="" />
                  <p className="type-body-md font-black text-white">{p.authorName || orgName}<span className={`text-white/30 font-bold ${TYPE.labelSm}`}> · {new Date(p.timestamp).toLocaleDateString()}</span></p>
                </div>
                ...
                <button onClick={() => uid ? togglePostLike(p.id) : alert('Sign in to react.')}
                  className={`tap flex items-center gap-1.5 ${TYPE.labelMd} font-black uppercase tracking-widest transition-colors`}
