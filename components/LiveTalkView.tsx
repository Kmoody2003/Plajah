...
const LiveTalkView: React.FC<LiveTalkViewProps> = ({ onBrowse }) => {
  ...
  const [inputSource, setInputSource] = useState('Default Mic');
  const [talkTab, setTalkTab] = useState<'CHAT' | 'ASSETS'>('CHAT');
  const [activeMeterId, setActiveMeterId] = useState<string | null>(null);

  const handleStartTalk = async () => { /* Implement start logic */ };
  const handleStopTalk = async () => { await endLiveTalk(activeTalk.id); };
  const toggleMuteSpeaker = (uid: string) => { /* Implement toggle mute logic */ };

  const [raisedHands, setRaisedHands] = useState<string[]>([]);

  useEffect(() => {
    if (activeTalk) {
      // Cast to include raisedHands even if not in type
  ...
  const handleApproveHand = async (uid: string) => {
    ...
    const newSpeakers = [...(activeTalk.speakers), {
      uid: uid,
      name: 'User', // Would ideally fetch name from UIDs
          ...
          <div className="flex items-center justify-between mb-4">
             ...
             {isHost && (
               <div className="flex gap-2">
                   <button onClick={handleStartTalk} className="text-[9px] font-black uppercase tracking-widest text-green-500">Start</button>
                   <button onClick={handleStopTalk} className="text-[9px] font-black uppercase tracking-widest text-red-500">Stop</button>
                   <select value={inputSource} onChange={e => setInputSource(e.target.value)} className="bg-black text-[9px] text-white">
        ...
        <div className="p-6 flex flex-col gap-6 border-b border-white/5 bg-black/20">
           <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-4">
                 ...
                 {activeTalk.speakers.filter(s => s.uid !== activeTalk.hostId).map(speaker => (
                    <div key={speaker.uid} className="relative group">
                       <div className="w-12 h-12 rounded-2xl overflow-hidden bg-white/5 border border-white/10 shadow-lg cursor-pointer">
                          <img src={speaker.photoURL} className="w-full h-full" />
                          {/* Live Meter (Visual Placeholder) */}
                          <div className={`absolute bottom-0 left-0 right-0 h-1 bg-green-500 transition-all ${speaker.uid === activeMeterId ? 'opacity-100' : 'opacity-0'}`} />
                       </div>
                       
                       {/* Host Controls */}
                       {isHost && (
                         <div className="absolute -top-2 -right-2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => toggleMuteSpeaker(speaker.uid)} className="p-1 bg-black rounded shadow">
           ...
           <div className="space-y-4">
              <div className="flex items-center justify-between">
                 ...
                 {!isHost && !activeTalk.speakers.find(s => s.uid === auth.currentUser?.uid) && (
                   <button onClick={handleRaiseHand} className="px-2 py-0.5 bg-[#00DAF3] text-black text-[8px] font-black rounded-lg hover:scale-105 transition-transform">Raise Hand</button>
                 )}
