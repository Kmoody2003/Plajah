    ...
    <div className="flex-1 p-6 lg:p-12 max-w-7xl mx-auto w-full">
      <div className="flex flex-col gap-6 mb-12">
        <div className="w-full">
          ...
          <div className="relative group mt-8 max-w-2xl">
            <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-white/30 group-focus-within:text-small-orange transition-all duration-300" size={20} />
            <input
              type="text"
              placeholder={mode === 'MOVIES_TV' ? "Search cinema & series..." : "Search videos..."}
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full bg-black/60 backdrop-blur-3xl border border-white/15 focus:border-white/40 rounded-2xl py-4 pl-14 pr-6 text-sm font-black uppercase tracking-widest text-white placeholder:text-white/20 outline-none shadow-[0_8px_32px_rgba(0,0,0,0.6),inset_0_1px_0_rgba(255,255,255,0.08)] focus:shadow-[0_8px_40px_rgba(0,0,0,0.8),inset_0_1px_0_rgba(255,255,255,0.12)] transition-all duration-300"
            />
            {searchTerm && (
              <button
