...
import { FeedItem, UserProfile, FeedPage, Game, Album, PostThemeBackground, LiveTalk } from '../types';
import { fetchFeed, fetchFollowedFeed, postToFeed, followUser, unfollowUser, isFollowing, deleteFeedItem, fetchUserProfile, fetchUserAlbums, fetchThemeBackgrounds, listenToActiveLiveTalks, updateUserProfile, searchUserProfiles, subscribeToComments, postComment } from '../services/backendService';
import { fetchNewsFromRSS } from '../services/rssService';
import { ArrowLeft, User, Music2, MessageSquare, Image as ImageIcon, Send, Play, UserPlus, UserMinus, Globe, Newspaper, Zap, TrendingUp, Reply, Trash2, Sparkles, Book, Disc, Gamepad2, Tv, Radio, Layers, ChevronLeft, ChevronRight, Maximize2, ExternalLink, Volume2, VolumeX, Pause, Plus, Check, X, Heart, Pen, Share2, Mic, Search, Users, Cloud, Smile, MoreHorizontal, Info } from 'lucide-react';
...
const FeedView: React.FC<FeedViewProps> = ({ onBack, currentUser, onVisitUser, onMessage, onSelectGame }) => {
  ...
  const [plajahFilter, setPlajahFilter] = useState<'ALL' | 'FOLLOWING'>('ALL');
  ...
  const [selectedSportsTab, setSelectedSportsTab] = useState<'HEADLINES' | 'SPORTS' | 'NBA' | 'NFL' | 'NHL' | 'MLB' | 'NCAA' | 'SCIENCE' | 'FINANCE'>('HEADLINES');
  ...
  return (
    <div className="flex-1 overflow-y-auto custom-scrollbar bg-transparent">
      <div className="p-4 md:p-12 max-w-full mx-auto w-full">
        {/* BOLDER HEADER */}
            ...
            <div className="flex flex-wrap items-center gap-4 px-4 mt-8">
              <div className="flex p-1 bg-white/5 rounded-full border border-white/10 mr-4">
                <button 
                  ...
                  className={`px-6 py-2 rounded-full text-[9px] font-black uppercase tracking-widest transition-all ${plajahFilter === 'ALL' ? 'bg-white text-black' : 'text-white/40 hover:text-white'}`}
                >
                  Global
                </button>
                <button 
                  onClick={() => setPlajahFilter('FOLLOWING')}
                  className={`px-6 py-2 rounded-full text-[9px] font-black uppercase tracking-widest transition-all ${plajahFilter === 'FOLLOWING' ? 'bg-white text-black' : 'text-white/40 hover:text-white'}`}
                >
                  Following
                </button>
              </div>
      ...
      {activeTab === 'LIVETALK' ? (
        ...
        <div className="px-6 lg:px-20 max-w-[1600px] mx-auto w-full space-y-12 pb-20">
           ...
           <div className="flex items-center gap-4 overflow-x-auto no-scrollbar pb-6 sticky top-0 bg-transparent z-40 backdrop-blur-sm">
             {['HEADLINES', 'SPORTS', 'NBA', 'NFL', 'NHL', 'MLB', 'NCAA', 'SCIENCE', 'FINANCE'].map((tab) => (
               <button
                 key={tab}
           ...
           {/* LIVE SCORES ROW */}
           {selectedSportsTab !== 'HEADLINES' && selectedSportsTab !== 'SCIENCE' && selectedSportsTab !== 'FINANCE' && sportsScores.length > 0 && (
             <div className="space-y-6">
                <div className="flex items-center gap-4">
                   <div className="w-1.5 h-1.5 bg-red-500 rounded-full animate-pulse shadow-[0_0_8px_red]" />
                   <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-white/40 italic">Live Frequency: {selectedSportsTab} Scores</h4>
                </div>
                <div className="flex gap-6 overflow-x-auto no-scrollbar py-4 -mx-6 px-6">
                   {sportsScores.map((event: any) => (
                     <div key={event.id} className="min-w-[320px] bg-white/5 border border-white/10 rounded-[2.5rem] p-8 flex flex-col gap-6 hover:bg-white/10 transition-all cursor-pointer group">
                        <div className="flex items-center justify-between">
                        ...
                        <div className="space-y-4">
                           {event.competitions[0].competitors.map((team: any) => (
                             <div key={team.id} className="flex items-center justify-between">
                                <div className="flex items-center gap-4">
                                   <img src={team.team.logo || null} alt="" className="w-8 h-8 object-contain opacity-80 group-hover:opacity-100 transition-opacity" />
                                   <span className={`text-sm font-black uppercase tracking-tight ${team.winner ? 'text-white' : 'text-white/40'}`}>
                                      {team.team.abbreviation}
                                   </span>
                                </div>
                                <span className={`text-xl font-black italic ${team.winner ? 'text-small-orange' : 'text-white'}`}>
                                   {team.score}
                                </span>
                             </div>
                           ))}
                        </div>
                        {event.competitions[0].notes.length > 0 && (
                          <p className="text-[8px] font-black uppercase tracking-widest text-[#00DAF3]/60 italic mt-2">{event.competitions[0].notes[0].headline}</p>
                        )}
                     </div>
                   ))}
                </div>
        ...
        <div className="pt-4 pb-20 max-w-[1700px] mx-auto w-full px-4 lg:px-8 flex flex-col xl:flex-row gap-8 lg:gap-16">
      ...
      {currentUser && (activeTab === 'GLOBAL' || activeTab === 'SOCIAL') && (
