...
import { ArrowLeft, ClipboardCheck, Flame, Send, Check, GraduationCap, RefreshCw, Trophy, Loader2 } from 'lucide-react';
...
import { completionPercent, type DigitalWorksheet } from '../services/worksheetDigitizer';
import { turnInWorksheet, fetchWorksheet } from '../services/worksheetAssignmentService';
...
const StudentAssignmentView: React.FC<Props> = ({ user, onBack, worksheet, worksheetId }) => {
  ...
  const doTurnIn = async () => {
    ...
    const res = await turnInWorksheet({
      ...
      sheet, studentId: user?.uid || 'demo-student', studentName: user?.displayName || 'You',
      answers,
        ...
        {tab === 'worksheet' && (
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.3fr) minmax(0, 1fr)', gap: 14 }}>
            <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 14, padding: 16 }}>
              <WorksheetFillable sheet={sheet} answers={answers} setAnswers={setAnswers} accent={T.orange} results={graded?.results} readOnly={!!graded} />
