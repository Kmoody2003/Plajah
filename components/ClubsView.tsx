...
const ClubsView: React.FC<ClubsViewProps> = ({ onBack, currentUser, onCreatePitchDeck }) => {
  ...
  if (selectedWcTeam) {
    return (
      <div className="min-h-screen bg-transparent text-[var(--text-primary)] max-w-4xl mx-auto px-4 sm:px-6 lg:px-12 pt-8 pb-32">
        <FanClubDetail
          team={selectedWcTeam}
          currentUser={currentUser}
            ...
            {filteredClubs.map(club => (
              ...
              >
                ...
                <div className="p-4 sm:p-8">
                  <p className="opacity-40 text-xs font-bold uppercase tracking-widest leading-relaxed mb-6 sm:mb-8 line-clamp-2">{club.description}</p>
      ...
      <AnimatePresence>
        {showCreateModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-6">
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9 }} className="bg-[#0a0a0a] border border-white/10 rounded-3xl p-5 sm:p-8 max-w-lg w-full max-h-[90vh] overflow-y-auto">
